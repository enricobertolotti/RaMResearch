...
# Return the max crop amount to the edges of the image
# Works with n-Dimensional arrays
# pos should have the same length as the amount of dimensions of array_dim
def get_crop_dim(array_dim, pos: Union[tuple, list, np.ndarray]):
    return [(pos[i], array_dim[i] - pos[i]) for i in range(len(array_dim))]


# Returns the minimum distance from the point to each edge of the array
def get_max_crop(array_dim, pos: Union[tuple, list, np.ndarray]):
    return [np.minimum(i1, i2) for i1, i2 in get_crop_dim(array_dim, pos)]
